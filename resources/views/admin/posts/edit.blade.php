    ...
    <form action="{{ route('admin.posts.update', $post->id) }}" enctype="multipart/form-data" method="POST"
        ...
        <div class="mb-4 w-75">
            ...
            <input type="text" class="form-control {{ $errors->has(" title") ? 'is-invalid' : '' }}" name="title"
                id="field_title" value="{{ old(" title") ?? $post->title }}">
        ...
        <div class="mb-4 w-75">
            ...
            <input type="text" class="form-control {{ $errors->has(" subtitle") ? 'is-invalid' : '' }}" name="subtitle"
                id="field_subtitle" value="{{ old(" subtitle") ?? $post->subtitle }}">
        ...
        <div class="mb-4 w-75">
            ...
            <input type="file" class="form-control {{ $errors->has(" coverImg") ? 'is-invalid' : '' }}" name="coverImg"
                id="field_coverImg" value="{{ old('coverImg') ?? $post->coverImg }}">
            ...
            <div>
                <div class="form-group">
                    ...
                    <select name="tags[]" class="form-control" multiple id="field_tags">
                        @foreach($tags as $tag)
                        <option value="{{$tag->id}}" @if ($post->tags->contains($tag)) selected @endif>
                            {{$tag->name}}
                        </option>
                        @endforeach
                    </select>
